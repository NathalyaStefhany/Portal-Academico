...
class SubjectService {
    ...
    async createSubject(
        ...
        const subject = new Subject(acronym, name, coursesIn, period, classes, requirements, credits);
    ...
    private verifySubjectTaken(acronym: string, historic: Historic, acronymClasses: Array<string>) {
    ...
    async getRequirementsTable(matriculationNumber: number) {
        ...
        let requirementTable = subjectsToverify.map(s => {
            subjectRequirement = {
                ...
                Credits: s.Credits,
                HistoricFlag: this.verifySubjectTaken(s.Acronym, historic, acronymClasses)
            }
        ...
        requirementTable = requirementTable.sort(
            function shortSubject(subject1: RequimentTable, subject2: RequimentTable){
